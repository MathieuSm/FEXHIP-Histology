...
def RandomROIs(Valid, Rand, Spacing):
...
def ExtractROIs(Image:np.array, Bone:np.array, N:int, ROIName:str):
    ...
    if np.sum(Valid) > 0:
        ...
        Patches = patchify(Image[:,Left:Right], patch_size=(ROISize, ROISize, 3), step=iStep)
...
def Main(N=12):
    ...
    # Set directories
    DataDir = Path(__file__).parent / '..' / '01_Data'
    Folders = sorted([F for F in DataDir.iterdir() if F.name.startswith('Donor')])

    # Create results directory
    ResDir = Path(__file__).parent / '..' / '03_Results' / 'ROIs_Selection'    
...
if __name__ == '__main__':
    ...
    Parser.add_argument('-N', '--Number', help='Number of ROIs', default=3, type=int)
